...
def main():
    try:
        ...
        logging.info(f"Total documents fetched: {total_docs}")
        print(f"\n[SUCCESS] Total documents fetched: {total_docs}\n")
        ...
        # ✅ Sort links by URL path
        links_sorted = sorted(links, key=lambda x: urlparse(x).path)

        # Save sorted links with timestamp
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        links_file = f"data/links_{timestamp}.txt"
        ...
        # Also save as 'links.txt' (latest overwrite)
        ...
        logging.info(f"Links saved to: {links_file}")
        print(f"\n[SAVED] Links saved to: {links_file}")
        print(f"Total links discovered (sorted): {len(links_sorted)}")
        logging.info("Link extraction completed successfully")
