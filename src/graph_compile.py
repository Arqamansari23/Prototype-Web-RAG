...
#                             2. If the message is about feedback,  suggestions, or comments, respond with:
...
class GraphBuilder:
    ...
    def __init__(self):
        ...
        # Create user interest in company tool
        self._create_user_interest_tool()
    ...
    def _create_retrieval_tool(self):
        ...
        try:
            ...
            self.retriever = self.vector_store.as_retriever(
                search_type="similarity",
                search_kwargs={"k": 5}  # Fixed value instead of config
            )
    ...
    def _create_greeting_feedback_tool(self):
        ...
        greeting_tool = Tool(
            name="greeting_and_feedback_handler",
            description="Use this tool ONLY for greeting messages (hi, hello, hey, good morning, etc.) and feedback-related queries (reviews, ratings, opinions, suggestions, comments). Do NOT use this tool for contact information requests like email, phone number, or address queries. Donot use this tool if user ask for Clients reviews, testimonials, or client relationships. This tool handles conversational starters and feedback requests.",
            func=greeting_feedback_handler
        )
    ...
    def _create_user_interest_tool(self):
        """Create user interest in company tool."""
        def user_interest_handler(user_message: str) -> str:
            """
            Handle user interest in company services and development inquiries.
            """
            print("---USER INTEREST IN COMPANY HANDLER---")
            
            # Use LLM-based prompt for user interest detection and response

            prompt = PromptTemplate(
                template="""You are a customer service representative for Genetech Solutions, a software development company.

                        Analyze the user's message to determine if they are expressing interest in company services or development opportunities.

                        User's message: {user_message}

                    Instructions:
                    Detect if the user is interested in any of the following areas:
                    ...
                    - Any development or technical services

                    If the message indicates interest in any of these areas, respond with:

                    "Thank you for your interest in Genetech Solutions' services! We specialize in software development, app development, AI solutions, and custom technology solutions.

                    To discuss your project requirements and explore how we can help you, please feel free to contact us:

                    📞 Pakistan: +92 21 3455 8425  
                    📞 USA: +1 734-519-1414  
                    📧 Email: info@genetech.co  
                    👩‍💼 LinkedIn: You can also connect with our COO, Shamim Rajani, on LinkedIn for direct consultation — https://www.linkedin.com/in/shamimrajani?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=android_app

                We'd be happy to discuss your project and provide a customized solution tailored to your needs."

                If the message doesn't clearly indicate interest in development services, respond with:

                "I'd be happy to help you learn more about our services. Could you please specify what type of solution or service you're interested in?"
        ...
        # Create the user interest tool
        user_interest_tool = Tool(
            name="user_interest_in_company",
            description="User interest in company like I want to develop software, app, AI solution. Use this tool when users express interest in development services, software solutions, app development, AI solutions, web development, custom software, technology consulting, or any technical services the company offers.",
            func=user_interest_handler
        )
        
        self.tools.append(user_interest_tool)
        self.logger.info("✅ Created user interest in company tool")
    ...
    def _grade_documents(self, state: AgentState) -> Literal["Output_Generator", "Handle_Irrelevant"]:
        ...
        # Get the last message to check if it's from greeting tool or user interest tool
        messages = state["messages"]
        last_message = messages[-1]
        
        # If the last message is from the greeting tool or user interest tool, skip grading
        if hasattr(last_message, 'tool_calls') and last_message.tool_calls:
            tool_call = last_message.tool_calls[0]
            if tool_call['name'] in ['greeting_and_feedback_handler', 'user_interest_in_company']:
                print("---DECISION: GREETING/FEEDBACK/USER INTEREST RESPONSE - SKIP GRADING---")
                return "Output_Generator"
    ...
    def _generate_response(self, state: AgentState):
        ...
        # Check if it's a greeting/feedback/user interest response that doesn't need RAG processing
        if isinstance(last_message.content, str) and any(phrase in last_message.content.lower() for phrase in 
                                                        ["hello! this is genetech solution", 
                                                         "thank you for wanting to provide feedback",
                                                         "this is genetech solution",
                                                         "thank you for your interest in genetech solution's services",
                                                         "i'd be happy to help you learn more about our services"]):
            print("---DIRECT GREETING/FEEDBACK/USER INTEREST RESPONSE---")
            return {"messages": [AIMessage(content=last_message.content)]}
        ...
        prompt = PromptTemplate(
                    template="""You are Genetech Solutions' intelligent AI assistant, designed to professionally and accurately respond to user queries about the company, its services, technologies, job openings, or any related topic.

                Use the retrieved context below to craft a clear, informative, and well-structured response. If the information is not available in the context, politely state that you don't know rather than guessing.

                        Your response should be:
                        - Professional and aligned with Genetech Solutions' tone.
                        - Detailed and organized using **only the relevant markdown-style headings**.
                        - Easy to read, with bullet points or short paragraphs where applicable.

                        Always start your response with:
                        **Genetech Solutions:**

                        Choose from the following headings based on relevance to the user's question:
                        - ## Overview
                        - ## Services & Expertise
                        - ## Technologies We Use
                        ...
                        Only include the headings that are necessary to answer the user's question. Do not include empty or irrelevant sections.
